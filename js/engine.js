...
function calculate() {
  ...
  let aul = false;
  let awlName = "";
  let originalAslForAwl = asl;

  if (fixSum > asl) {
    aul = true;
    asl = fixSum;
    if (asl === 27) {
      awlName = " — الميمبرية (Mimbarriyya)";
    }
  }
  ...
  const awlDisplay = aul ? `العول${awlName}` : "";
...
`;
  ...
  document.getElementById("assetsDetail").innerHTML = `
    ...
    ${finalRows.map(r => {
    const a = net > 0 ? Math.round(r.sahm / asl * net) : 0;
